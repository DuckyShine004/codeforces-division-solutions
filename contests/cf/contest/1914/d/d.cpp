...
vi get_best(const vi &a, int n) {
    int amx, bmx, cmx, ai, bi, ci;

    ai = bi = ci = 0;
    amx = bmx = cmx = -1;

    fors(i, 0, n) {
        // Check if we have a first max or that we need to update max
        if (amx == -1 || a[i] > amx) {
            // Shift maxes
            cmx = bmx;
            ci = bi;
            ...
            amx = a[i];
            ai = i;
        } else if (bmx == -1 || a[i] > bmx) {
            cmx = bmx;
            ci = bi;

            bmx = a[i];
            bi = i;
        } else if (cmx == -1 || a[i] > cmx) {
            cmx = a[i];
            ci = i;
        } else {
            continue;
